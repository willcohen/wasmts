...
(def gen-lineseg-pair
  ;; [receiver argument] pairs for the nullable LineSegment methods
  ;; (intersection / lineIntersection / project), bound through a manual.edn
  ;; :gen-tuple hint. Half are built to cross, so the comparison of two
  ;; non-nil results runs; the other half are independent, and those mostly
  ;; check that both sides return nil.
