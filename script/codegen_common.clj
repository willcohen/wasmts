...
(defn- expand-inline-tags
  ...
  (-> s
      ...
      ;; Anything left ({@value}, {@inheritDoc}, {@docRoot}) keeps its
      ;; content and drops the braces.
      (str/replace #"\{@\w+\s*([^}]*)\}" "$1")))
...
(defn- decode-entities
  "Decode the HTML entities that JTS uses. `&amp;` is last, so `&amp;lt;`
   ...
   One rule per entity that JTS 1.20 writes, and no more. An entity JTS
   adds later fails that test with the block that carries it, which is a
   better signal than a rule nobody can tell is doing anything. `&phi;`
   stays although it reaches no emitted block today: it is in
   `MathUtil.quasirandom(double)`, which loses to its own overload in
   `dedup-by-path`, not to anything stable."
  [s]
  (-> s
      (str/replace #"&lt;" "<")
      (str/replace #"&gt;" ">")
...
(def supported-shapes
  "Legacy keyword shapes still in scope. Everything else routes through
   the structured `{:kind ...}` shapes in `templated-kinds`; these cover
   the constant fields and the few bespoke dispatch bodies in emit_api."
  #{:geometry-apply-filter
    :geometry-get-coordinate-sequence
    :linemerger-add :linemerger-get-merged-line-strings
    :pm->type-friendly
    :static-int-field :static-double-field :static-char-field})
