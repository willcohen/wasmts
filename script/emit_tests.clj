(ns emit-tests
  ...
  (:require
   ...
   [emit-tests-compare :refer [coord-array-equal-form generic-return]]))
        ...
        (case ret
          ...
          `(~'cond
             ...
             (~'let [~'wasmts-xyzs (~'rpc/wasmts-coord-array-xyzs ~'wasmts-handle)]
               ~(coord-array-equal-form 'jts-result 'wasmts-xyzs 1e-6))))]
    `(~'defspec ~(spec-name k) 100
    ...
    `(~'defspec ~(spec-name k) 100
       (~'prop/for-all [~'coords ~'gen-coord-array]
         (let [~'handles        (~'rpc/jts->wasmts-coord-array ~'coords)
               ...
               ~'wasmts-xyzs    (~'rpc/wasmts-coord-array-xyzs ~'wasmts-handle)]
           (try
             ~(coord-array-equal-form 'jts-result 'wasmts-xyzs 1e-6)
             (finally
               (~'rpc/release-all-coords! ~'handles)
