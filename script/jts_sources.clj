(ns jts-sources
  "Locate and parse the JTS sources jar once. `javadoc-index` reads the doc
   comments off the CompilationUnits below; `source-param-names` reads the
   parameter names off the same units. Both find the jar through
   `jar-path`, and `compilation-units` memoizes the parse, so one run opens
   and parses the jar one time.
